...
query.param <- list(country_of_residence = "USA", country_of_origin = "SYR", year = 2013)
a <- GET('http://data.unhcr.org/api/stats/persons_of_concern.json', query = query.param)
body2 <- fromJSON(content(a, 'text'))
str(body2)
...
qurey.p <- list(country_of_residence = "TUR", country_of_origin = "SYR", population_type_code = "RF")
response <- GET("http://data.unhcr.org/api/stats/time_series_all_years.json?", query = qurey.p)
refugees.turky <- fromJSON(content(response, "text"))
refugees.turky <- select(refugees.turky, year, turkey = value)
as.numeric(refugees.turky$turkey)
as.numeric(refugees.usa$usa)

left_join(refugees.usa, refugees.turky, by = 'year')%>%
  mutate(more = usa > turkey)
